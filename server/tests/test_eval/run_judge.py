...
# The two dimensions this runner judges (the §5 D5 register + correct-Arabic legs).
JUDGE_DIMENSIONS = ("register", "correct_arabic")
...
def run_judge(path: pathlib.Path = _GOLD_SET_PATH) -> dict[str, Any]:
    """Score the gold set on register + correct-Arabic via the Vertex Gemini judge.

    Returns per-dimension {scores, mean} plus the full `score_eval_set` report (so the model-free
    D1/D2 legs ride along for the gate). Calling this REACHES Vertex (integration; `make eval`)."""
    ...
    register_scores = judge_dimension(model, rubric, "register", cases)
    arabic_scores = judge_dimension(model, rubric, "correct_arabic", cases)

    report = score_eval_set(path, register_scores=register_scores, arabic_scores=arabic_scores)
    report["_judge_model"] = JUDGE_MODEL
    report["_threshold"] = JUDGE_THRESHOLD
    report["_gate_passes"] = gate_passes(report)
    return report


def main() -> int:
    """`make eval` entrypoint for the judge leg — exits NON-ZERO if a judged dimension is below
    threshold (D-09) or the model-free D1 faithfulness gate is < 100% (D-08)."""
    report = run_judge()
    register = report["register"]
    arabic = report["correct_arabic"]
    print(
        f"Vertex LLM-judge ({JUDGE_MODEL}, judge != coach) — "
        f"register: {register.get('score')}, correct_arabic: {arabic.get('score')} "
        f"(threshold {report['_threshold']})"
    )
    if not report["_gate_passes"]:
        print("EVAL GATE FAILED — D1 faithfulness < 100% OR a judged dimension below threshold.")
        return 1
