...
## User Setup Required
**Terminal human gate — Cloud Run re-deploy (Task 3, not executed here).** The updated tool schema takes effect in production only after the `qalam-tutor` Cloud Run service is re-deployed from `server/`. Per project convention each prod deploy needs fresh explicit owner wording + the firebase/gcloud credentials, so the executor does NOT run it. The change is forward-safe (additive optional params only, no request-DTO change → no 422 window). On-device verification (a real UAT build, NOT `--dart-define=DEMO=true`): complete a clean baa pass and confirm the Teacher's Margin WHY varies per attempt (the client presentation is finished by 18-16; this half verifies the server is live).
