...
- 2026-07-18 baa masking asymmetry: (1) baa's canvas is 242px narrower (Teacher's Margin row is `_isAgentPath`-gated) → less stretch → most attempts land in the FUZZY zone, which the soft verdict passes. Today's baa deterministic wire fields: 22 certainlyCorrect / 15 fuzzy (several hugging the wrong edge: 0.10, 0.20, 0.27) / 5 certainlyWrong — consistent with a systematic stretch penalty. (2) The June-30 "scorer false-failed correct writing" episode was this same defect, undiagnosed — it motivated the tcc/tcw widenings and the since-superseded image-judge experiment.
- 2026-07-18 POST-RESOLUTION CORRECTION (owner follow-up): pass/fail ownership verified at HEAD — the June-30 image-judge was SUPERSEDED by Phase 17 D-A. The deterministic scorer owns pass/fail + star for ALL letters incl. baa (`exercise_scaffold._onResult` applies the scorer verdict via `applyResult` before the brain runs; "the brain only enriches the WORDS (D-A)"; zero strokeImage/image_judge code in lib/). The root_cause line below saying "AI judge owns its verdict since 2026-06-30" is wrong on that point; baa's masking was the narrower canvas + fuzzy-passes, per the evidence above.
