...
class _FakeProgressRepository implements ProgressRepository {
  ...
  final StreamController<Set<String>> _never = StreamController<Set<String>>();
...
GoRouter _makeRouter() => GoRouter(
  ...
  routes: <RouteBase>[
    ...
    GoRoute(
      path: '/journey',
      builder: (context, state) => const Scaffold(body: Text('Journey Screen')),
    ),
    ...
    GoRoute(
      path: '/parent',
      builder: (context, state) => const Scaffold(body: Text('Parent Screen')),
    ),
  ],
);
...
Finder _cardArabic(String text) => find.descendant(
  of: find.byKey(const Key('todaysLessonCard')),
  matching: find.byWidgetPredicate((w) => w is ArabicText && w.text == text),
);
  ...
  group('HomeScreen', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          ...
          findsNothing,
          reason:
              'the hardcoded "Layla" greeting must be replaced by the '
              'profile nickname.',
        );
        expect(
          ...
          findsOneWidget,
          reason:
              'the resolved nickname label for nick_star ("نجمة") must render.',
        );
        expect(
        ...
        expect(find.byKey(const Key('homeJourneyPreview')), findsOneWidget);
        expect(
          find.byKey(const Key('homeMascotEncouragement')),
          findsOneWidget,
        );
        expect(find.textContaining('saved 20'), findsNothing);
      },
    );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          find.textContaining('stars this week'),
          findsNothing,
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          find.byKey(const Key('parentNavItem')),
          findsOneWidget,
        ...
        expect(
          ...
          findsNothing,
          reason:
              'The grown-ups control speaks through its icon — no written '
        ...
        expect(
          ...
          findsNothing,
          reason:
              'Parent is unlocked in Phase 9 — no nav item is "Coming soon".',
        );
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          find.text('YOUR LETTERS'),
          findsOneWidget,
          reason: 'all-mastered eyebrow (D-11).',
        );
        expect(
          find.text('You\'ve mastered all your letters.'),
          findsOneWidget,
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          find.byKey(const Key('todaysLessonCard')),
          findsOneWidget,
          reason: 'the card shell renders while loading.',
        );
        expect(
          find.textContaining('The Letter'),
          findsNothing,
    ...
    testWidgets(
      ...
      'the card still renders a Start (Test 9)',
      (WidgetTester tester) async {
        final router = _makeRouter();
        await tester.pumpWidget(
          _buildHome(
    ...
    testWidgets(
      ...
      'no entrance fade wrappers (Test 11)',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          _buildHome(
            router: _makeRouter(),
    ...
    // -----------------------------------------------------------------------
    testWidgets('entrance: card fades/slides in over durSlow, glyph fades over '
    ...
    testWidgets(
      ...
      'replaying the entrance (Test 13)',
      (WidgetTester tester) async {
