// Quick task 260720-wcs (Task 3) — THE taa LIVE-PATH mastery→advance PROOF.
//
// F2-interim dormancy (Task 1) removed taa's 10 reach-ahead word/grammar nodes,
// leaving 7 all-essential letter-FORM nodes. This pins that the resulting 7-node
// taa graph can actually be COMPLETED to the star and that the journey advances to
// thaa — the SAME live-path shape the 260720-up4 baa test proves, over taa's own
// bundled graph (off disk, 7 nodes). It drives a scored PASS through the REAL
// selection path (WriteSurface.onResult) and taps the pass CTA; the shell's
// _advanceSelection routes to Mastery, whose post-frame _recordMasteryIfMet
// persists the row. taa declares NO presentedEssentials → the controller uses the
// FULL-graph essential check (all 7 nodes).
//
...
const _resumeNode = 'taa.traceLetter.isolated';

Map<String, dynamic> _json(String path) =>
    jsonDecode(File(path).readAsStringSync()) as Map<String, dynamic>;

/// The REAL bundled taa graph, parsed off disk (File, not rootBundle) — 7 nodes
/// after the 260720-wcs F2-interim dormancy. The SAME graph the app ships.
CurriculumGraph _loadGraph() => CurriculumGraph.fromJson(
      _json('assets/curriculum/graphs/taa.json').cast<String, Object?>(),
    );
...
Letter _taa() {
  ...
  return Letter(
    id: 'taa',
    char: 'ت',
    name: const LetterName(ar: 'تاء', display: 'taa'),
    introOrder: 3,
    forms: const LetterForms(
        isolated: 'ت', initial: 'تـ', medial: 'ـتـ', final_: 'ـت'),
...
/// The taa unit shell data — NO presentedEssentials (the controller then uses the
/// FULL-graph essential check over all 7 taa nodes). The resume-node exercise is
/// the isolated trace the presenter renders.
LetterUnitData _taaData() => LetterUnitData(
      unit: const LetterUnit(
        letterId: 'taa',
        // No presentedEssentials → full-graph essential mastery check.
        sections: [
          UnitSection(id: 'meet', exercises: ['taa.teachCard.meet']),
          UnitSection(id: 'watchTrace', exercises: [_resumeNode]),
          UnitSection(id: 'forms', exercises: ['taa.writeLetter.writeForm']),
      ...
      exercises: {
        for (final e in [
          Exercise(
            ...
            type: 'traceLetter',
            skill: 'formation',
            prompt: const [SayPart('Trace taa.')],
            surface: const Surface(
                mode: 'trace', unit: 'glyph', guideForm: 'isolated', demo: true),
            expected: const Answer(glyph: GlyphAnswer(char: 'ت', form: 'isolated')),
...
void main() {
  ...
  testWidgets(
      'the real 7-node taa graph reaches the star through live selection and the '
      'journey advances to thaa (F2-interim dormancy, no direct recordMasteryIfMet)',
      (tester) async {
    ...
    final container = ProviderContainer(overrides: [
      ...
      // The REAL 7-node taa graph, off disk (no rootBundle stall).
      ...
      letterUnitDataProvider('taa').overrideWith((ref) async => _taaData()),
    ...
    expect(graph.nodes, hasLength(7),
        reason: 'the bundled taa graph is 7 nodes after the 260720-wcs dormancy');
    expect(graph.essentialNodes, hasLength(7),
        reason: 'all 7 remaining taa nodes are essential (the 10 reach-ahead word '
            'cards were made dormant)');

    // Realistic progression state: alif + baa already MASTERED (so taa is today's
    // letter), and EVERY taa essential node seeded at its owner threshold — so the
    // child has genuinely completed taa's core and the FIRST scored moment finds
    // mastery met.
    ...
    await tester.pumpWidget(
      UncontrolledProviderScope(
        container: container,
        child: const MaterialApp(home: LetterUnitScreen(letterId: 'taa')),
      ),
    );
    await tester.pumpAndSettle();

    final controller =
        container.read(letterUnitControllerProvider('taa').notifier);
    ...
    final picked = await _awaitPumping(tester, pending!, what: 'nextReady(taa)');
    expect(picked, isNull,
        reason: 'full-graph essential mastery is met → selection TERMINATES '
            '(returns null → route to Mastery); the 7-node taa unit finishes');
    ...
    //     thaa (the next letter by introOrder) as today's letter.
    final expectedNext = _nextByIntroOrder('taa'); // thaa
    final snapshot = await _awaitPumping(
      tester,
      container.read(progressionProvider.future),
      what: 'progressionProvider(taa)',
    );
    expect(snapshot.masteredLetterIds, containsAll(<String>['alif', 'baa', 'taa']));
    final todayLetter = snapshot.today?.items
        .where((i) => i.type == 'letter')
        .map((i) => i.ref)
        .firstOrNull;
    expect(todayLetter, expectedNext,
        reason: 'after mastering taa, today\'s lesson advances to $expectedNext');
