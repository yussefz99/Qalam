...
void main() {
  ...
  group('PinService hash / verify (T-09-01, S1-11)', () {
    test('setPin then verify with the SAME 4-digit PIN returns true', () async {
      ...
      expect(await pinService.verify(db, '1234'), isTrue,
          reason: 'the correct PIN must verify');
    });

    test('verify with a WRONG PIN returns false', () async {
      await pinService.setPin(db, '1234');
      expect(await pinService.verify(db, '9999'), isFalse,
          reason: 'an incorrect PIN must not verify');
    });

    test('the stored value is NEVER the plaintext PIN (hashed, T-09-01)',
        () async {
      await pinService.setPin(db, '1234');
    ...
    test(
        'two setPin calls with the SAME PIN produce DIFFERENT stored hashes '
    ...
    test('isPinSet is false before setPin, true after', () async {
      ...
      await pinService.setPin(db, '1234');
      expect(await pinService.isPinSet(db), isTrue,
  ...
  group('persisted brute-force cooldown (T-09-02, S1-11)', () {
    test('a fresh database has no cooldown', () async {
      expect(await pinService.remainingCooldown(db), anyOf(isNull, Duration.zero),
    ...
    test('registerSuccess clears any pending cooldown', () async {
      ...
      expect(remaining, anyOf(isNull, Duration.zero),
          reason: 'a correct PIN must clear the throttle');
    });

    test(
        '5 consecutive failures set a non-null cooldown that SURVIVES a '
      ...
      expect(beforeRestart! > Duration.zero, isTrue,
      ...
      expect(afterRestart! > Duration.zero, isTrue,
          reason: 'force-quitting must NOT bypass the brute-force cooldown '
